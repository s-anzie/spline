...
Spline est une plateforme de coordination, d'orchestration et de supervision permettant à des agents IA et des humains de collaborer efficacement sur des projets complexes.

Spline ne cherche pas à remplacer les fournisseurs de modèles de langage.

Spline fournit une infrastructure commune permettant à plusieurs agents, provenant de fournisseurs différents, de travailler ensemble selon un protocole partagé, un état partagé et des règles d'exécution déterministes.
...
Le système repose sur un état partagé persistant.

Les agents ne doivent jamais être la source de vérité.

---

Control Plane

Toutes les décisions critiques passent par le Control Plane.

Aucun agent ne décide seul de l'état global.

---

Provider Agnostic

Tous les fournisseurs sont considérés comme interchangeables.

Le protocole interne de Spline est indépendant des modèles utilisés.

---

Deterministic Infrastructure

Les modèles sont probabilistes.

L'infrastructure qui les entoure doit être déterministe.

---

Human Supervision

L'humain peut intervenir à tout instant.

Le système ne suppose jamais une autonomie absolue.
...
Les agents collaborent selon un protocole imposé.

Jamais selon des conventions implicites.
...
Un projet progresse vers des résultats.

Pas uniquement vers des tâches.

---

Observable

Chaque action doit pouvoir être observée.
...
Spline est organisé autour de deux catégories de composants.

Le premier groupe décide.
...
Le Control Plane ne réalise jamais directement le travail.
...
Le Control Plane est le cerveau du système.

Il possède la totalité de l'état métier.
...
Le Worker Runtime est installé sur une machine.
...
Chaque agent est exécuté dans une session indépendante.
...
Spline considère Git comme une entité métier.
...
Les modifications Git sont pilotées par Spline.

Jamais directement par les agents.

---

## 3.6 Validation Engine

Une tâche n'est jamais terminée parce qu'un agent le déclare.
...
Toutes les communications passent par un bus d'événements.

Les événements représentent des faits.

Jamais des intentions.
...
Le Policy Engine applique les règles.
...
La source de vérité est toujours le Control Plane.
...
Ces composants représentent un état dérivé.

Leur état peut être reconstruit.

Le Control Plane ne peut pas être reconstruit à partir d'eux.

---

# 4. Domain Model

Le Domain Model définit les objets métier fondamentaux de Spline.

Chaque composant du système est construit autour de ces entités.

Une entité représente une réalité métier persistante.
...
Le Workspace représente l'unité d'isolation principale.

Toutes les ressources appartiennent à exactement un Workspace.

Aucun état métier n'est partagé directement entre deux Workspaces.
...
Le Repository représente un dépôt Git.

Spline considère Git comme une ressource métier.

Git n'est jamais piloté directement par un agent.
...
Un Worktree représente une copie de travail isolée.

Chaque tâche possède son propre Worktree.

Deux tâches ne partagent jamais le même environnement Git.
...
Un Goal représente un résultat observable.

Un Goal décrit ce qui doit être atteint.
...
Une Task représente une unité atomique de travail.

Une Task possède un seul responsable.

Les collaborateurs éventuels interviennent via des tâches séparées.
...
Un Run représente une exécution logique.

Une même tâche peut être exécutée plusieurs fois.

Chaque tentative crée un nouveau Run.
...
Une Attempt représente une tentative d'exécution.
...
Une Validation représente une preuve.

Une tâche n'est jamais terminée sans preuve.
...
Une AgentSession représente une instance vivante d'un agent.

Une session est éphémère.

L'Agent est permanent.
...
Le Lease protège une exécution.

Si le Worker disparaît, le Lease expire.

La tâche peut être reprise.
...
Une Decision représente un choix métier.

Elle permet d'expliquer pourquoi une action a été prise.
...
Une tâche bloquée ne progresse plus.
...
Représente une information destinée à un acteur.

Une Notification possède plusieurs destinataires.

Chaque destinataire possède son propre état.
...
Toute action importante génère une entrée d'audit.
...
Le Control Plane constitue le cœur de Spline.

Il est responsable de l'état global du système.

Aucun composant ne peut modifier directement l'état métier sans passer par le Control Plane.

Le Control Plane ne réalise jamais le travail opérationnel.
...
Le Validation Engine décide si une tâche peut être considérée comme terminée.

Une tâche est validée uniquement si :
...
Le Lock Manager garantit l'exclusivité.
...
Les locks sont toujours temporaires.
...
Chaque modification importante génère une entrée.
...
Toutes les communications passent par le bus.

Le bus est responsable de :
...
Le Control Plane est la seule autorité.

Le Runtime ne peut jamais modifier directement :
...
Il soumet une requête.

Le Control Plane décide.

---

# 6. Worker Runtime

Le Worker Runtime est le composant installé sur une machine.

Il exécute les décisions prises par le Control Plane.
...
Si aucun heartbeat n'est reçu avant expiration du délai :

Le Worker est considéré comme indisponible.

---

## 6.5 Lease

Toute tâche active possède un Lease.

Le Lease protège l'exécution.
...
Le Runtime maintient uniquement un état local.

En cas de divergence :

Le Control Plane fait autorité.

Le Runtime doit se resynchroniser.
...
L'Agent Runtime représente l'environnement d'exécution logique d'un agent.

Il constitue l'interface entre le provider IA et Spline.

Le Runtime traduit les décisions du Control Plane en actions exécutables.

Le Runtime ne possède jamais l'état métier.
...
Chaque session commence par un Sync.
...
Chaque couche enrichit la suivante.

Le contexte est reconstruit à chaque exécution.
...
Informations utiles au Goal courant.

---

### Task Memory

Informations strictement nécessaires à la tâche.
...
Les outils ne sont jamais appelés directement.

Chaque appel passe par le Runtime.
...
Spline ne stocke jamais des prompts arbitraires.

Le Runtime construit le prompt à partir :
...
Les réponses libres ne modifient jamais l'état.

Pour modifier le système, l'agent doit produire une Action.
...
Le Repository Engine constitue le cœur du travail logiciel.

Git est considéré comme une ressource métier.

Les agents ne manipulent jamais directement Git.

Ils demandent des opérations.

Le Repository Engine les réalise.
...
Aucune tâche ne travaille directement sur :
...
Le merge n'est jamais réalisé par un agent.

Le Repository Engine décide.
...
Le Scheduling Engine est responsable de l'allocation du travail.

Son objectif est de transformer un ensemble de Goals en exécutions ordonnées.

Il ne réalise jamais le travail.

Il décide où et quand il sera exécuté.
...
Une tâche échouée peut être relancée.
...
Chaque tâche possède un délai maximal.
...
Spline définit un protocole obligatoire.

Tous les agents suivent exactement le même cycle.

Aucun provider n'est autorisé à définir son propre protocole.
...
Avant d'agir, l'agent produit un plan.
...
Il ne décide jamais lui-même que son travail est terminé.
...
Toutes les communications utilisent des événements structurés.
...
Tout blocage devient une entité métier.
...
Le Validation Engine est responsable de déterminer objectivement si une tâche, un Goal ou une modification du système peut être considéré comme terminé.

Les agents ne peuvent jamais déclarer eux-mêmes une réussite.

Ils soumettent des résultats.

Le Validation Engine décide.
...
Une validation peut être réalisée par un autre agent spécialisé.
...
Le Policy Engine définit les règles du Workspace.

Les politiques sont déclaratives.

Les agents ne peuvent pas les contourner.
...
Le Lock Manager protège les ressources critiques.

Les Locks sont distribués.

Ils possèdent toujours une durée de vie.
...
Chaque Lock est associé à un Lease.

Sans Lease valide :

Le Lock est automatiquement supprimé.

---

## 13.4 Conflits

Deux demandes concurrentes produisent :
...
Ils ne peuvent jamais rester permanents.
...
Le Event Bus constitue le système nerveux de Spline.

Tous les composants communiquent uniquement au travers d'événements.
...
Chaque événement est publié une seule fois.

Les consommateurs décident ensuite de leur traitement.

---

## 14.4 Replay

Le système peut rejouer les événements.
...
Les Artifacts représentent tous les objets produits par Spline.

Ils constituent la mémoire durable du projet.
...
Chaque modification produit une nouvelle version.
...
Le Memory System fournit aux agents un accès cohérent à la connaissance du projet.

La mémoire n'est jamais une simple base vectorielle.

Spline distingue plusieurs niveaux de mémoire répondant à des besoins différents.
...
Aucune mémoire ne constitue la source de vérité.

La source de vérité reste le Domain Model.
...
Elle ne doit jamais contenir d'information critique.

---

## 16.9 Indexation

Toutes les mémoires sont indexées.

Recherche possible selon :
...
Spline est conçu pour être entièrement observable.

Aucun composant ne fonctionne comme une boîte noire.
...
Spline fournit des tableaux de bord pour :
...
Toutes les opérations passent par RBAC.
...
Les secrets ne sont jamais transmis directement aux agents.

Le Runtime fournit uniquement les secrets nécessaires à la tâche.
...
Les sessions peuvent être exécutées dans :
...
L'API constitue l'interface officielle du système.
...
Chaque Worker envoie régulièrement :
...
Le Runtime tente une reconnexion.
...
Le Scheduler détecte les Leases expirés.

Les tâches sont réassignées.

Les artefacts restent disponibles.

---

## 22.3 Checkpoints

Les longues exécutions produisent des Checkpoints.

Ils permettent une reprise.
...
Ils créent toujours une nouvelle Attempt.

---

# 23. Deployment

Spline peut être déployé :
...
Le Control Plane est horizontalement scalable.

Les Workers peuvent être ajoutés dynamiquement.

---

## 23.3 Haute disponibilité

Les services critiques peuvent être répliqués.

La perte d'un Worker ne provoque pas la perte des tâches.
