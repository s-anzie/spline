...
export type RuntimeHealth = { machines:{total:number;online:number;stale:number;offline:number}; sessions:{active:number;stale:number}; commands:{pending:number;stuck:number}; computedAt:string };
