...
function toHttpError(error: DomainError): Error {
  if (error instanceof WorkspaceNotFoundError || error instanceof MachineNotFoundError) {
...
export class MachineController {
  ...
  async list(@Param("workspaceId") workspaceId: string) {
    const machines = await this.listMachinesByWorkspaceUseCase.execute(workspaceId);
  ...
  async link(@Param("workspaceId") workspaceId: string, @Param("machineId") machineId: string) {
    const result = await this.linkMachineToWorkspaceUseCase.execute({ workspaceId, machineId });
    if (result.isFailure) {
