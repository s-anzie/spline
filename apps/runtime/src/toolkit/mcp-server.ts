...
const tools: Tool[] = [
  ...
  { name: "spline_create_task", description: "Create a scoped task with an explicit title and optional description.", inputSchema: { type: "object", properties: { title: stringProperty("Task title"), description: stringProperty("Scope and acceptance criteria"), priority: stringProperty("LOW, MEDIUM, HIGH, or CRITICAL") }, required: ["title"] }, method: "POST", path: () => workspace("/tasks"), body: (i) => ({ title: i.title ?? null, description: i.description ?? null, priority: i.priority ?? "MEDIUM" }) },
  ...
  { name: "spline_launch_agent", description: "Manager-only: launch a contributor for an assigned task with explicit acceptance criteria.", inputSchema: { type: "object", properties: { agentId: stringProperty("Contributor UUID"), machineId: stringProperty("Online linked machine UUID"), taskId: stringProperty("Assigned task UUID"), instruction: stringProperty("Outcome, scope, constraints, acceptance criteria, validation") }, required: ["agentId", "machineId", "taskId", "instruction"] }, method: "POST", path: () => workspace("/agent-sessions"), body: (i) => i },
  { name: "spline_delegate_task", description: "Manager-only: coherently create, assign, and launch a contributor task; compensates task creation if launch fails.", inputSchema: { type: "object", properties: { title: stringProperty("Task title"), description: stringProperty("Scope and acceptance criteria"), priority: stringProperty("LOW, MEDIUM, HIGH, or CRITICAL"), agentId: stringProperty("Contributor UUID"), machineId: stringProperty("Online linked machine UUID"), instruction: stringProperty("Outcome, constraints, acceptance criteria, and validation") }, required: ["title", "description", "agentId", "machineId", "instruction"] }, method: "POST", path: () => workspace("/collaboration/delegate"), body: (i) => i },
